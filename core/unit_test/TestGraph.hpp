...
#define KOKKOS_TEST_GRAPH_CAPTURE_DEFINE_ADD(_capture_, _backend_)                                        \
  ...
  static auto add(const N& pred, const Kokkos::_backend_& exec, const DstType& dst, SrcTypes&&... srcs) { \
    /* TODO Use C++ 20 "Allow pack expansion in lambda init-capture" feature. */                          \
...
template <typename Exec>
std::enable_if_t<GraphNodeTypes<Exec>::support_capture == false, void>
test_graph_capture() {
  GTEST_SKIP() << "The graph backend for "
               << Kokkos::Impl::TypeInfo<Exec>::name()
               << " does not support capture.";
}

template <typename Exec>
std::enable_if_t<GraphNodeTypes<Exec>::support_capture == true, void>
test_graph_capture() {
...
TEST(TEST_CATEGORY, graph_capture) { test_graph_capture<TEST_EXECSPACE>(); }
