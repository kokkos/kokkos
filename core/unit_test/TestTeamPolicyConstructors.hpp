...
struct TestTeamPolicyCTADS {
  ...
  // TEST_EXECSPACE deduces to TeamPolicyTestExecSpace

  static_assert(std::is_same_v<TeamPolicyTestExecSpace,
                               decltype(Kokkos::TeamPolicy(es, i, i))>);
  static_assert(std::is_same_v<TeamPolicyTestExecSpace,
                               decltype(Kokkos::TeamPolicy(es, i, i, i))>);
  static_assert(
      std::is_same_v<TeamPolicyTestExecSpace,
                     decltype(Kokkos::TeamPolicy(es, i, Kokkos::AUTO))>);
  static_assert(
      std::is_same_v<TeamPolicyTestExecSpace,
                     decltype(Kokkos::TeamPolicy(es, i, Kokkos::AUTO, i))>);
  static_assert(std::is_same_v<TeamPolicyTestExecSpace,
                               decltype(Kokkos::TeamPolicy(es, i, Kokkos::AUTO,
                                                           Kokkos::AUTO))>);
  static_assert(
      std::is_same_v<TeamPolicyTestExecSpace,
                     decltype(Kokkos::TeamPolicy(es, i, i, Kokkos::AUTO))>);
};
