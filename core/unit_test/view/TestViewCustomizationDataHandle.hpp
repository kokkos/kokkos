...
// Test internal ways of customizing the data handle for applications such as
// resilience

namespace TestDataHandle {
template <class ElementType, class MemorySpace>
class TestCustomDataHandle
    : public Kokkos::Impl::ReferenceCountedDataHandle<ElementType,
                                                      MemorySpace> {
 public:
  using base_t =
  ...
  explicit TestCustomDataHandle(
      Kokkos::Impl::SharedAllocationRecord<void, void>* rec, int val)
...
template <class ElementType, class MemorySpace>
struct IsTestCustomDataHandle<TestCustomDataHandle<ElementType, MemorySpace>>
    : std::true_type {};
...
inline constexpr struct CustomDataHandleTag_t {
} custom_data_handle_tag;

template <class LayoutType, class DeviceType, class MemoryTraits>
constexpr auto customize_view_arguments(
    Kokkos::Impl::ViewArguments<CustomDataType, LayoutType, DeviceType,
                                MemoryTraits>) {
  return custom_data_handle_tag;
}

template <class ElementType, class MDSpanExtents, class MDSpanLayoutType,
          class MemorySpace, class NestedAccessor>
class TestCustomDataHandleAccessor {
 public:
  ...
  using offset_policy =
      TestCustomDataHandleAccessor<ElementType, MDSpanExtents, MDSpanLayoutType,
                                   MemorySpace,
...
template <typename DataHandleType, typename MDSpanExtents,
...
KOKKOS_INLINE_FUNCTION constexpr DataHandleType data_handle_from_allocation(
    ...
    const typename MDSpanLayoutType::template mapping<
        MDSpanExtents>& /* mapping */,
    const Kokkos::Impl::SpaceAwareAccessor<
        MemorySpace,
        TestCustomDataHandleAccessor<
            ElementType, MDSpanExtents, MDSpanLayoutType, MemorySpace,
            Kokkos::default_accessor<ElementType>>>& /* accessor */) {
...
struct MDSpanViewTraits<Traits, TestDataHandle::CustomDataHandleTag_t,
                        LayoutType> {
  using index_type =
      typename MDSpanViewTraits<Traits, void, LayoutType>::index_type;
  using extents_type =
      typename MDSpanViewTraits<Traits, void, LayoutType>::extents_type;
  using mdspan_layout_type =
      typename MDSpanViewTraits<Traits, void, LayoutType>::mdspan_layout_type;
  using accessor_type = Kokkos::Impl::SpaceAwareAccessor<
      typename Traits::memory_space,
      TestDataHandle::TestCustomDataHandleAccessor<
          typename Traits::value_type, extents_type, mdspan_layout_type,
          typename Traits::memory_space,
...
KOKKOS_INLINE_FUNCTION constexpr auto ptr_from_data_handle(
    const TestDataHandle::TestCustomDataHandle<ElementType, MemorySpace>&
        handle) {
  return handle.get();
}
...
struct IsReferenceCountedDataHandle<
    TestDataHandle::TestCustomDataHandle<ElementType, MemorySpace>>
    : std::true_type {};
}  // namespace Kokkos::Impl

using view_custom_data_handle_type =
    Kokkos::View<TestDataHandle::CustomDataType*>;

static_assert(TestDataHandle::IsTestCustomDataHandle<
                  view_custom_data_handle_type::data_handle_type>::value,
              "view data handle type must be a resilient handle");
