...
class GraphNodeRef {
 ...
 public:
  ...
  /// Capture the backend operations that calling the passed function object
  /// with the passed execution space instance enqueues into the underlying
  /// backend queue.
  /// @note The function object will be stored by the underlying graph
  ///       implementation, thereby ensuring that any captured data is kept
  ///       alive until the graph is destroyed.
  /// @note The parameter @p exec can be used for multi-GPU graphs.
  ///       The captured nodes will be assigned to the device associated with
  ///       @p exec.
  /// @warning It's the user's responsability to ensure that theere is no
  ///          illegal operation going on while capturing.
  /// @todo Support label.
