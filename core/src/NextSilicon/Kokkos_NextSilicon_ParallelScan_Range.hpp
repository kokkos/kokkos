...
class ParallelScanNextSilicon {
 ...
 private:
  __attribute__((noinline)) void execute_internal(
    ...
    nextapi::parallel_for(begin, end, parallel_function<false>,
                          num_partial_prod, &m_functor, &m_final_reducer,
                          &m_policy, partial_prod);

    scan_partial_prod(m_final_reducer, m_result_ptr, partial_prod,
                      num_partial_prod);

    nextapi::parallel_for(begin, end, parallel_function<true>, num_partial_prod,
                          &m_functor, &m_final_reducer, &m_policy,
                          partial_prod);
  }

  template <bool Final>
  static void parallel_function(IndexType thread_index,
                                uint32_t num_partial_prod,
