...
namespace Impl {
  ...
  struct SubViewTypeDeduction {
    ...
    using data_type = typename DataTypeFromExtents<value_type,extents_type>::type;
    using type = Kokkos::View<data_type,
                              layout_type,
                              device_type,
                              memory_traits>;
  };
...
auto subview(const ViewType& v, Args...args) {
  return typename Impl::SubViewTypeDeduction<ViewType,decltype(subspan(v.get_mdspan(),args...))>::type(v,args...);
}
} // namespace Kokkos
