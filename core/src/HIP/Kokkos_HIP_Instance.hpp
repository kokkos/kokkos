...
// shared_resource.check_if_involved(std::move(lock_for_involvement), stream);
...
struct SharedResourceLocking {
  ...
  void check_if_involved(std::unique_lock<std::mutex> lock,
                         hipStream_t stream) {
    KOKKOS_ENSURES(lock.owns_lock());
    KOKKOS_ENSURES((lock.mutex() == std::addressof(m_mutex)));
