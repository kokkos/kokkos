...
hipStream_t get_deep_copy_stream() {
  ...
  if (s == nullptr) {
    KOKKOS_IMPL_HIP_SAFE_CALL(
        (HIPInternal::singleton().hip_stream_create_wrapper(&s)));
  }
  return s;
}
}  // namespace
...
void DeepCopyHIP(void* dst, void const* src, size_t n) {
  KOKKOS_IMPL_HIP_SAFE_CALL((HIPInternal::singleton().hip_memcpy_async_wrapper(
...
void DeepCopyAsyncHIP(void* dst, void const* src, size_t n) {
  ...
  Kokkos::Tools::Experimental::Impl::profile_fence_event<HIP>(
      ...
      [&]() {
        KOKKOS_IMPL_HIP_SAFE_CALL(
            (HIPInternal::singleton().hip_stream_synchronize_wrapper(s)));
      });
}
}  // namespace Impl
}  // namespace Kokkos
