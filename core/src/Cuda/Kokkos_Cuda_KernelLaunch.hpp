...
const cudaFuncAttributes& get_cuda_kernel_func_attributes(
  ...
  auto wrap_get_attributes = [&]() -> cudaFuncAttributes {
    ...
    // Clang compilers in the CI were throwing an
    // error complaining of ambiguous address of overloaded function
    // "cudaFuncGetAttributes" (templated cudaAPI function).
    // Manually call cudaAPI functions as a workaround.
    ...
    KOKKOS_IMPL_CUDA_SAFE_CALL(
        (CudaInternal::singleton()
             .cuda_api_interface<cudaFuncAttributes*, KernelFuncPtr>(
...
inline void configure_shmem_preference(const KernelFuncPtr& func,
  ...
  auto set_cache_config = [&] {
    KOKKOS_IMPL_CUDA_SAFE_CALL(
        (CudaInternal::singleton()
             .cuda_api_interface<KernelFuncPtr, cudaFuncAttribute, int>(
...
struct CudaParallelLaunchKernelInvoker<
  ...
  inline static void create_parallel_launch_graph_node(
    ...
    if (!Impl::is_empty_launch(grid, block)) {
      ...
      KOKKOS_IMPL_CUDA_SAFE_CALL(
          (cuda_instance->cuda_api_interface<cudaGraphNode_t*, cudaGraph_t,
...
struct CudaParallelLaunchKernelInvoker<
  ...
  static void invoke_kernel(DriverType const& driver, dim3 const& grid,
    ...
    KOKKOS_IMPL_CUDA_SAFE_CALL(
        (cuda_instance->cuda_api_interface<void*, const void*, size_t,
...
struct CudaParallelLaunchKernelInvoker<
  ...
  static void invoke_kernel(DriverType const& driver, dim3 const& grid,
    ...
    KOKKOS_IMPL_CUDA_SAFE_CALL((cuda_instance->cuda_api_interface<cudaEvent_t>(
    ...
    KOKKOS_IMPL_CUDA_SAFE_CALL((
        cuda_instance->cuda_api_interface<const void*, const void*, size_t,
    ...
    KOKKOS_IMPL_CUDA_SAFE_CALL(
        (cuda_instance->cuda_api_interface<cudaEvent_t, cudaStream_t>(
...
struct CudaParallelLaunchImpl<
  ...
  inline static void launch_kernel(const DriverType& driver, const dim3& grid,
    ...
    if (!Impl::is_empty_launch(grid, block)) {
      ...
      KOKKOS_IMPL_CUDA_SAFE_CALL(
          (cuda_instance->cuda_api_interface(&cudaGetLastError)));
      cuda_instance->fence(
