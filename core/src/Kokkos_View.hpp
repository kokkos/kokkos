...
#define KOKKOS_IMPL_VIEW_HOOKS_NVCC_WORKAROUND 1
  ...
  View(const View& other) : base_t{other} {
    if constexpr (has_hooks_policy) {
      ...
      // we need to make sure the template is actually instantiated or we may
      // have a mismatch on code those host sees vs what the device sees
      [[maybe_unused]] auto* copy_construct_ptr =
          &hooks_policy::template copy_construct<View>;
  ...
  View(const View& other)
    requires(has_hooks_policy)
      : base_t{other} {
    // Has to be outside KOKKOS_IF_ON_HOST
    // we need to make sure the template is actually instantiated or we may have
    // a mismatch on code those host sees vs what the device sees
    [[maybe_unused]] auto* copy_construct_ptr =
        &hooks_policy::template copy_construct<View>;
    KOKKOS_IF_ON_HOST((copy_construct_ptr(*this, other);))
  }
  ...
  View(View&& other) : base_t{std::move(static_cast<base_t&&>(other))} {
    if constexpr (has_hooks_policy) {
      ...
      // we need to make sure the template is actually instantiated or we may
      // have a mismatch on code those host sees vs what the device sees
      [[maybe_unused]] auto* move_construct_ptr =
          &hooks_policy::template move_construct<View>;
  ...
  View& operator=(const View& other) {
    ...
    if constexpr (has_hooks_policy) {
      ...
      // we need to make sure the template is actually instantiated or we may
      // have a mismatch on code those host sees vs what the device sees
      [[maybe_unused]] auto* copy_assign_ptr =
          &hooks_policy::template copy_assign<View>;
      KOKKOS_IF_ON_HOST(
    ...
    requires(has_hooks_policy)
  {
    ...
    [[maybe_unused]] auto* copy_assign_ptr =
        &hooks_policy::template copy_assign<View>;
    KOKKOS_IF_ON_HOST((if (&other != this) { copy_assign_ptr(*this, other); }))
...
// differently
  ...
  View& operator=(View&& other) {
    ...
    if constexpr (has_hooks_policy) {
      ...
      // we need to make sure the template is actually instantiated or we may
      // have a mismatch on code those host sees vs what the device sees
      [[maybe_unused]] auto* move_assign_ptr =
          &hooks_policy::template move_assign<View>;
      KOKKOS_IF_ON_HOST(
    ...
    requires(has_hooks_policy)
  {
    ...
    [[maybe_unused]] auto* move_assign_ptr =
        &hooks_policy::template move_assign<View>;
    KOKKOS_IF_ON_HOST((if (&other != this) { move_assign_ptr(*this, other); }))
