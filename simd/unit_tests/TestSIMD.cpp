...
class shift_right {
 public:
  ...
  template <typename T>
  KOKKOS_INLINE_FUNCTION auto on_device(T&& a, int b) const {
    return a >> b;
  }
  template <typename T>
  KOKKOS_INLINE_FUNCTION auto on_device(
      T&& a,
      Kokkos::Experimental::simd<
          std::int32_t, Kokkos::Experimental::simd_abi::scalar>& b) const {
    return a >> b;
  }
};
...
KOKKOS_INLINE_FUNCTION void device_check_shift_on_one_loader(
    ShiftOp shift_op, DataType test_vals[], const unsigned int shift_by[],
    std::size_t n) {
  using simd_type             = Kokkos::Experimental::simd<DataType, Abi>;
  std::size_t constexpr width = simd_type::size();
  Loader loader;
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    for (std::size_t lane = 0; lane < width; ++lane) {
      expected_result[lane] =
          shift_op.on_device(DataType(simd_vals[lane]), shift_by[i]);
    }

    simd_type const computed_result =
        shift_op.on_device(simd_vals, shift_by[i]);
...
KOKKOS_INLINE_FUNCTION void device_check_shift_by_lanes_on_one_loader(
  ...
  for (std::size_t lane = 0; lane < width; ++lane) {
    expected_result[lane] =
        shift_op.on_device(DataType(simd_vals[lane]), shift_by[lane]);
  }
