...
inline void host_check_equality(
  ...
  for (std::size_t i = 0; i < nlanes; ++i) {
    // gcc release with debug symbols build with -O3 doesn't seem to properly load simd values into simd vectors until values are directly accessed. Placing in an harmless intermediate check to ensure that values are properly laoded into simd vectors.
