...
template <typename T, typename... Flags,
          std::enable_if_t<!std::is_same_v<simd_abi::Impl::host_fixed_native,
                                           simd_abi::scalar>,
                           bool> = false>
KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION
    basic_simd<T, simd_abi::Impl::host_fixed_native>
    simd_unchecked_load(const T* ptr,
                        simd_flags<Flags...> flag = simd_flag_default) {
  return simd_unchecked_load<basic_simd<T, simd_abi::Impl::host_fixed_native>>(
      ptr, flag);
}
...
simd_unchecked_load(const T* ptr,
                    simd_flags<Flags...> flag = simd_flag_default) {
  static_assert(
      std::is_same_v<simd_abi::Impl::host_fixed_native, simd_abi::scalar>);
