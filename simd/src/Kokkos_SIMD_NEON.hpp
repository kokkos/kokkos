...
class neon_mask<Derived, 32, 4> {
 ...
 public:
  ...
  operator[](std::size_t i) const {
    ...
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION Derived operator||(
        neon_mask const& lhs, neon_mask const& rhs) noexcept {
    ...
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator==(
        neon_mask const& lhs, neon_mask const& rhs) noexcept {
      uint64x2_t const elementwise_equality =
          vceqq_u64(lhs.m_value, rhs.m_value);
      uint32x2_t const narrow_elementwise_equality =
          vqmovn_u64(elementwise_equality);
      uint64x1_t const overall_equality_neon =
          vreinterpret_u64_u32(narrow_elementwise_equality);
      uint64_t const overall_equality = vget_lane_u64(overall_equality_neon, 0);
      return Derived(overall_equality == 0xFFFFFFFFFFFFFFFFULL);
    }
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator!=(
        neon_mask const& lhs, neon_mask const& rhs) noexcept {
  ...
  class neon_mask<Derived, 32, 2> {
   ...
   public:
    ...
    operator uint32x2_t() const noexcept {
      return m_value;
    }

    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator&&(
        neon_mask const& lhs, neon_mask const& rhs) const noexcept {
...
#ifdef KOKKOS_ENABLE_DEPRECATED_CODE_4
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION bool operator==(
        neon_mask const& other) const noexcept {
    ...
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator==(
        neon_mask const& lhs, neon_mask const& rhs) noexcept {
      uint32x2_t const elementwise_equality =
          vceq_u32(lhs.m_value, rhs.m_value);
      uint64x1_t const overall_equality_neon =
          vreinterpret_u64_u32(elementwise_equality);
      uint64_t const overall_equality = vget_lane_u64(overall_equality_neon, 0);
      return Derived(overall_equality == 0xFFFFFFFFFFFFFFFFULL);
  ...
  class neon_mask<Derived, 32, 4> {
   ...
   public:
    ...
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator||(
        neon_mask const& lhs, neon_mask const& rhs) const {
    ...
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator==(
        neon_mask const& lhs, neon_mask const& rhs) noexcept {
      uint32x4_t const elementwise_equality =
          vceqq_u32(lhs.m_value, rhs.m_value);
      uint64x2_t const overall_equality_neon =
          vreinterpretq_u64_u32(elementwise_equality);
      return Derived((overall_equality_neon[0] == 0xFFFFFFFFFFFFFFFFULL) &&
                     (overall_equality_neon[1] == 0xFFFFFFFFFFFFFFFFULL));
    }
    KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION friend Derived operator!=(
        neon_mask const& lhs, neon_mask const& rhs) const {
...
class basic_simd_mask<T, simd_abi::neon_fixed_size<2>>
 ...
 public:
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION constexpr basic_simd_mask() noexcept =
      default;
