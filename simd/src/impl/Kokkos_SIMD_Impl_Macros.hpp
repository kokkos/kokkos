...
#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE(FN, DST_TYPE, SRC_TYPE, MASK_TYPE, ...) \
...
#define KOKKOS_SIMD_IMPL_DEFINE_FN_DEVICE(RET_TYPE, FN, ARG_TYPE, ...) \
  ...
  KOKKOS_SIMD_IMPL_DEFINE_FN(RET_TYPE, FN, [[maybe_unused]] ARG_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_GEN_FN_HOST(RET_TYPE, FN, ...) \
  template <typename G> \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN(RET_TYPE, FN, G&&, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_GEN_FN_DEVICE(RET_TYPE, FN, ...) \
  template <typename G> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN(RET_TYPE, FN, [[maybe_unused]] G&&, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FN_HOST(RET_TYPE, FN, FROM, ABI, ...) \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FN(RET_TYPE, FN, FROM, ABI, simd_host_tag, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FN_DEVICE(RET_TYPE, FN, FROM, ABI, ...) \
  template <typename T, typename Abi> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FN(RET_TYPE, FN, FROM, ABI, simd_device_tag, __VA_ARGS__)
...
#define KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FALLBACK_DEFN_HOST(IMPL_OPS, TO, ABI)         \
...
#define KOKKOS_SIMD_IMPL_DEFINE_CONVERSION_FALLBACK_DEFN_DEVICE(IMPL_OPS, TO, ABI)         \
  template <typename From>                                                     \
  KOKKOS_FORCEINLINE_FUNCTION                                                  \
...
#define KOKKOS_SIMD_IMPL_DEFINE_LOAD_HOST(RET_TYPE, FN, SRC_TYPE, ...) \
  ...
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_LOAD(RET_TYPE, FN, SRC_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_LOAD_DEVICE(RET_TYPE, FN, SRC_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_LOAD(RET_TYPE, FN, [[maybe_unused]] SRC_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_LOAD_HOST(RET_TYPE, FN, SRC_TYPE, MASK_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_LOAD(RET_TYPE, FN, SRC_TYPE, MASK_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_LOAD_DEVICE(RET_TYPE, FN, SRC_TYPE, MASK_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_LOAD(RET_TYPE, FN, [[maybe_unused]] SRC_TYPE, [[maybe_unused]] MASK_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_STORE_HOST(FN, DST_TYPE, SRC_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_STORE(FN, DST_TYPE, SRC_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_STORE_DEVICE(FN, DST_TYPE, SRC_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_STORE(FN, [[maybe_unused]] DST_TYPE, SRC_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE_HOST(FN, DST_TYPE, SRC_TYPE, MASK_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE(FN, DST_TYPE, SRC_TYPE, MASK_TYPE, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE_DEVICE(FN, DST_TYPE, SRC_TYPE, MASK_TYPE, ...) \
  template <typename... Flags> \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE(FN, DST_TYPE, SRC_TYPE, MASK_TYPE, __VA_ARGS__)
...
#define KOKKOS_SIMD_IMPL_DEFINE_UNARY_OP_DEVICE(RET_TYPE, FN, ...) \
  KOKKOS_SIMD_IMPL_DEFINE_FN_DEVICE(RET_TYPE, FN, [[maybe_unused]] vector_type, __VA_ARGS__)
...
#define KOKKOS_SIMD_IMPL_DEFINE_SHIFT_SCALAR_HOST(RET_TYPE, FN, ...) \
  KOKKOS_IMPL_HOST_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN_SHIFT_SCALAR(RET_TYPE, FN, vector_type, __VA_ARGS__)

#define KOKKOS_SIMD_IMPL_DEFINE_SHIFT_SCALAR_DEVICE(RET_TYPE, FN, ...) \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN_SHIFT_SCALAR(RET_TYPE, FN, [[maybe_unused]] vector_type, __VA_ARGS__)
...
#define KOKKOS_SIMD_IMPL_DEFINE_BINARY_OP_DEVICE(RET_TYPE, FN, ...) \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN_2ARGS(RET_TYPE, FN, [[maybe_unused]] vector_type, __VA_ARGS__)
...
#define KOKKOS_SIMD_IMPL_DEFINE_TERNARY_MATH_OP_DEVICE(RET_TYPE, FN, ...) \
  KOKKOS_FORCEINLINE_FUNCTION \
  KOKKOS_SIMD_IMPL_DEFINE_FN_3ARGS(RET_TYPE, FN, [[maybe_unused]] vector_type, __VA_ARGS__)
...
FN(Experimental::basic_simd_mask<T, Experimental::simd_abi::avx2_fixed_size<N>> const& a, Args const&... args) \
  requires requires { \
    Experimental::Impl::simd_native_ops<T, Experimental::simd_abi::avx2_fixed_size<N>, Experimental::Impl::simd_backend_t>::FN(a, args...); \
  } \
{ \
  using impl_ops = Experimental::Impl::simd_native_ops<T, Experimental::simd_abi::avx2_fixed_size<N>, Experimental::Impl::simd_backend_t>; \
  using simd_type = Experimental::basic_simd<T, Experimental::simd_abi::avx2_fixed_size<N>>; \
...
template <typename... Flags> \
KOKKOS_FORCEINLINE_FUNCTION basic_simd<DATA_TYPE, \
                                                ABI>  \
simd_##PREFIX##_load( \
    const DATA_TYPE* ptr, \
    basic_simd_mask<DATA_TYPE, ABI> const& mask, \
    simd_flags<Flags...> flag = simd_flag_default) { \
  return basic_simd<DATA_TYPE, ABI>(ptr, mask, \
                                                                 flag); \
}

#define KOKKOS_SIMD_IMPL_DEFINE_EXPLICIT_MASKED_LOAD_FN(PREFIX, DATA_TYPE, ABI) \
template <typename SimdType, typename... Flags> \
  requires std::same_as<typename SimdType::abi_type, ABI> \
KOKKOS_FORCEINLINE_FUNCTION \
    basic_simd<DATA_TYPE, ABI> \
...
#define KOKKOS_SIMD_IMPL_DEFINE_STORE_FN(PREFIX, DATA_TYPE, ABI, IMPL_FN)                   \
  template <typename... Flags>                     \
  KOKKOS_FORCEINLINE_FUNCTION                                                 \
  void simd_##PREFIX##_store(                                       \
      basic_simd<DATA_TYPE, ABI> const& simd,                \
      DATA_TYPE* ptr,                                                                 \
      ...
      {                        \
    using impl_ops = Impl::simd_native_ops<DATA_TYPE, ABI, Impl::simd_backend_t>; \
    impl_ops::IMPL_FN(ptr, simd, flag);                                       \
  }

#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE_FN(PREFIX, DATA_TYPE, ABI, IMPL_FN)                   \
  template <typename... Flags>                     \
  KOKKOS_FORCEINLINE_FUNCTION                                                 \
  void simd_##PREFIX##_store(                                       \
      basic_simd<DATA_TYPE, ABI> const& simd,                \
...
#define KOKKOS_SIMD_IMPL_DEFINE_LOAD_STORE_FN_AVX2(DATA_TYPE, ABI) \
  ...
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_STORE_FN(unchecked, DATA_TYPE, ABI, masked_store) \
...
#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_GATHER_FROM(                                 \
...
#define KOKKOS_SIMD_IMPL_DEFINE_MASKED_SCATTER_TO_AVX2(                                 \
...
#define KOKKOS_SIMD_IMPL_DEFINE_MEMORY_PERMUTE_FN_AVX2(DATA_TYPE, MASK_DATA_TYPE, ABI) \
  KOKKOS_SIMD_IMPL_DEFINE_GATHER_FROM(unchecked, DATA_TYPE, ABI) \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_GATHER_FROM(unchecked, DATA_TYPE, MASK_DATA_TYPE, ABI) \
  KOKKOS_SIMD_IMPL_DEFINE_GATHER_FROM(partial, DATA_TYPE, ABI) \
  KOKKOS_SIMD_IMPL_DEFINE_MASKED_GATHER_FROM(partial, DATA_TYPE, MASK_DATA_TYPE, ABI) \
