...
FUNCTION(KOKKOS_ADD_COMPILE_TEST TEST_NAME)
  CMAKE_PARSE_ARGUMENTS(PARSE
    "INCLUDE_CORE;INCLUDE_CONTAINERS"
    ""
    "SOURCES"
    ${ARGN})
  IF(KOKKOS_HAS_TRILINOS)
  ...
  ELSE()
    ...
    ELSE()
      ADD_EXECUTABLE(${EXE_NAME} ${PARSE_SOURCES})
      VERIFY_EMPTY(KOKKOS_ADD_COMPILE_TEST ${PARSE_UNPARSED_ARGUMENTS})

      IF(PARSE_INCLUDE_CORE)
        # Do everything but the link dependencies
        GET_TARGET_PROPERTY(CORE_DIRS kokkoscore INTERFACE_INCLUDE_DIRECTORIES)
        TARGET_INCLUDE_DIRECTORIES(${EXE_NAME} PUBLIC ${CORE_DIRS})
        GET_TARGET_PROPERTY(CORE_FLAGS kokkoscore INTERFACE_COMPILE_OPTIONS)
        TARGET_COMPILE_OPTIONS(${EXE_NAME} PUBLIC ${CORE_FLAGS})
        GET_TARGET_PROPERTY(CORE_FEATURES kokkoscore INTERFACE_COMPILE_FEATURES)
        TARGET_COMPILE_FEATURES(${EXE_NAME} PUBLIC ${CORE_FEATURES})
      ENDIF()

      IF(PARSE_INCLUDE_CONTAINERS)
        GET_TARGET_PROPERTY(CTRS_DIRS kokkoscontainers INTERFACE_INCLUDE_DIRECTORIES)
        TARGET_INCLUDE_DIRECTORIES(${EXE_NAME} PUBLIC ${CTRS_DIRS})
        GET_TARGET_PROPERTY(CTRS_FLAGS kokkoscontainers INTERFACE_COMPILE_OPTIONS)
        TARGET_COMPILE_OPTIONS(${EXE_NAME} PUBLIC ${CTRS_FLAGS})
        GET_TARGET_PROPERTY(CTRS_FEATURES kokkoscontainers INTERFACE_COMPILE_FEATURES)
        TARGET_COMPILE_FEATURES(${EXE_NAME} PUBLIC ${CTRS_FEATURES})
      ENDIF()
    ENDIF()
  ENDIF()
ENDFUNCTION()
