...
FUNCTION(kokkos_compilation)
    ...
    SET(_KOKKOS_LAUNCH_COMMAND "${Kokkos_COMPILE_LAUNCHER} ${Kokkos_NVCC_WRAPPER} ${CMAKE_CXX_COMPILER}")
    IF(COMP_GLOBAL)
        # if global, don't bother setting others
        GET_PROPERTY(_EXISTING_LAUNCH_COMPILE GLOBAL PROPERTY RULE_LAUNCH_COMPILE)
        GET_PROPERTY(_EXISTING_LAUNCH_LINK    GLOBAL PROPERTY RULE_LAUNCH_LINK)
        SET_PROPERTY(GLOBAL PROPERTY RULE_LAUNCH_COMPILE "${_EXISTING_LAUNCH_COMPILE} ${_KOKKOS_LAUNCH_COMMAND}")
        SET_PROPERTY(GLOBAL PROPERTY RULE_LAUNCH_LINK    "${_EXISTING_LAUNCH_LINK} ${_KOKKOS_LAUNCH_COMMAND}")
    ELSE()
        FOREACH(_TYPE PROJECT DIRECTORY TARGET SOURCE)
            ...
            IF(COMP_${_TYPE})
                ...
                GET_PROPERTY(_EXISTING_LAUNCH_COMPILE ${_TYPE} ${COMP_${_TYPE}} PROPERTY RULE_LAUNCH_COMPILE)
                GET_PROPERTY(_EXISTING_LAUNCH_LINK    ${_TYPE} ${COMP_${_TYPE}} PROPERTY RULE_LAUNCH_LINK)
                SET_PROPERTY(${_TYPE} ${COMP_${_TYPE}} PROPERTY RULE_LAUNCH_COMPILE "${_EXISTING_LAUNCH_COMPILE} ${_KOKKOS_LAUNCH_COMMAND}")
                SET_PROPERTY(${_TYPE} ${COMP_${_TYPE}} PROPERTY RULE_LAUNCH_LINK    "${_EXISTING_LAUNCH_LINK} ${_KOKKOS_LAUNCH_COMMAND}")
            ENDIF()
        ENDFOREACH()
    ENDIF()
ENDFUNCTION()
