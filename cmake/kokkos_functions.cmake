...
FUNCTION(COMPILER_SPECIFIC_OPTIONS_HELPER_HELPER)
  SET(COMPILERS PGI XL DEFAULT Cray Intel Clang AppleClang IntelClang GNU NVIDIA HIP)
  ...
  FOREACH(COMP ${COMPILERS})
    IF (NOT PARSE_${COMPILER})
      SET(PARSE_${COMPILER} ${PARSE_DEFAULT})
    ELSE IF("${PARSE_${COMPILER}" STREQUAL "NO-VALUE-SPECIFIED")
      SET(PARSE_${COMPILER}) #empty
    ENDIF()
  ENDFOREACH()
  ...
  IF (PARSE_PREFIX)
    LIST(LENGTH PARSE_${COMPILER} NUM_FLAGS)
    IF (NUM_FLAGS GREATER 1)
      IF (CMAKE_VERSION VERSION_LESS "3.13.0")
        MESSAGE(FATAL_ERROR "Multiple -Xcompiler flags not supported with CMake ${CMAKE_VERSION}"
                " - please upgrade to at least 3.13")
      ENDIF()
      FOREACH(FLAG ${PARSE_${COMPILER}})
        LIST(APPEND COMPILER_SPECIFIC_FLAGS_TMP "SHELL:${PREFIX} ${FLAG}")
      ENDFOREACH()
    ENDIF()
  ELSE()
...
FUNCTION(COMPILER_SPECIFIC_OPTIONS_HELPER_HELPER)
  CMAKE_PARSE_ARGUMENTS(
    ...
    ""
    ""
    "HOST_ONLY;HOST_DEVICE"
    ${ARGN})

  IF (PARSE_HOST_ONLY)
    # these flags are only supported on a host compiler
    COMPILER_SPECIFIC_OPTIONS_HELPER_HELPER(
      COMPILER_ID ${KOKKOS_CXX_COMPILER_ID}
      ${PARSE_UNPARSED_ARGUMENTS}
      ${PARSE_HOST_ONLY}
    )
  ENDIF()

  IF (PARSE_HOST_DEVICE)
    # these flags are supported on a host compiler
    # even if the host compiler is a -ccbin arg to a device compiler
    COMPILER_SPECIFIC_OPTIONS_HELPER_HELPER(
      PREFIX -Xcompiler
      COMPILER_ID ${KOKKOS_HOST_COMPILER_ID} #make the decision based on host compiler
      ${PARSE_UNPARSED_ARGUMENTS}
      ${PARSE_HOST_DEVICE}
    )
  ENDIF()

  IF (NOT PARSE_HOST_ONLY AND NOT PARSE_HOST_DEVICE)
    # there is no special logic needed
    COMPILER_SPECIFIC_OPTIONS_HELPER_HELPER(
      COMPILER_ID ${KOKKOS_CXX_COMPILER_ID}
      ${PARSE_UNPARSED_ARGUMENTS}
    )
