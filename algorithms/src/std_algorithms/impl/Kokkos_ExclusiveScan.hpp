...
OutputIteratorType
exclusive_scan_default_op_exespace_impl(const std::string& label,
  ...
  using func_type  = std::conditional_t<
    ::Kokkos::is_detected<ex_scan_has_reduction_identity_sum_t,
      ...
      ExclusiveScanDefaultFunctorForKnownNeutralElement<
          ...
          OutputIteratorType>,
      ExclusiveScanDefaultFunctorWithValueWrapper<ExecutionSpace, index_type, ValueType,
...
OutputIteratorType
exclusive_scan_custom_op_exespace_impl(const std::string& label,
				       const ExecutionSpace& ex,
				       InputIteratorType first_from,
				       InputIteratorType last_from,
				       OutputIteratorType first_dest,
...
exclusive_scan_default_op_team_impl(const TeamHandleType& teamHandle,
				    InputIteratorType first_from,
				    InputIteratorType last_from,
				    OutputIteratorType first_dest,
				    ValueType init_value)
{
 ...
 "The team-level impl of Kokkos::Experimental::exclusive_scan currently does not support types without reduction identity");
...
template <class TeamHandleType, class InputIteratorType,
          class OutputIteratorType, class ValueType, class BinaryOpType>
KOKKOS_FUNCTION OutputIteratorType
exclusive_scan_custom_op_team_impl(const TeamHandleType& teamHandle,
				   InputIteratorType first_from,
				   InputIteratorType last_from,
				   OutputIteratorType first_dest,
				   ValueType init_value,
				   BinaryOpType bop)
{
  ...
  static_assert(::Kokkos::is_detected_v<ex_scan_has_reduction_identity_sum_t, ValueType>,
 "The team-level impl of Kokkos::Experimental::exclusive_scan currently does not support types without reduction identity");
