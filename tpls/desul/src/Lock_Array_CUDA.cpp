...
std::unordered_map<int, int32_t*> CUDA_SPACE_ATOMIC_LOCKS_DEVICE_h = {};
std::unordered_map<int, int32_t*> CUDA_SPACE_ATOMIC_LOCKS_NODE_h   = {};
...
void init_lock_arrays_cuda(int device_id) {
  ...
  check_error_and_throw_cuda(error_set_device,
                             "init_lock_arrays_cuda: cudaSetDevice");
  auto error_malloc1 =
      cudaMalloc(&CUDA_SPACE_ATOMIC_LOCKS_DEVICE_h[device_id],
  ...
  auto error_malloc2 =
      cudaMallocHost(&CUDA_SPACE_ATOMIC_LOCKS_NODE_h[device_id],
                     sizeof(int32_t) * (CUDA_SPACE_ATOMIC_MASK + 1));
  ...
  check_error_and_throw_cuda(error_sync1,
                             "init_lock_arrays_cuda: post mallocs");
  init_lock_arrays_cuda_kernel<<<(CUDA_SPACE_ATOMIC_MASK + 1 + 255) / 256,
                                 256>>>();
  auto error_sync2 = cudaDeviceSynchronize();
  check_error_and_throw_cuda(error_sync2,
                             "init_lock_arrays_cuda: post init kernel");
}
